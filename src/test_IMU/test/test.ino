...
void loop() {
  ...
  accAngleX = (atan(AccY / sqrt(pow(AccX, 2) + pow(AccZ, 2))) * 180 / PI) - 76.83; // AccErrorX ~(0.58) See the calculate_IMU_error()custom function for more details
  accAngleY = (atan(-1 * AccX / sqrt(pow(AccY, 2) + pow(AccZ, 2))) * 180 / PI) + 0.53; // AccErrorY ~(-1.58)
  // === Read gyroscope data === //
  ...
  GyroX = GyroX - 498.82; // GyroErrorX ~(-0.56)
  GyroY = GyroY - 1.78; // GyroErrorY ~(2)
  GyroZ = GyroZ - 499.66; // GyroErrorZ ~ (-0.8)
